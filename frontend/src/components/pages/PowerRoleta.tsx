...
export default function PowerRoleta() {
  ...
  const powerSpins = profile?.powerSpins || 0; // Giros manuais extras
  ...
  const girosDisponiveis = Math.min(3, girosRestantesSemanais + powerSpins);
  ...
  const girar = useCallback(async () => {
    ...
    setSpinning(false);
  }, [spinning, girosDisponiveis, profile, refreshProfile, config.prizes, girosUsados, powerSpins, weeklyTotal]);
