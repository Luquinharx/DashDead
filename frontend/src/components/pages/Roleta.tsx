...
const PRIZES = [
  { id: 1, name: '500k Gold', chance: 80, color: 'text-amber-500', icon: '💰' },
  { id: 2, name: '1.5m Gold', chance: 15, color: 'text-red-500', icon: '💎' },
  { id: 3, name: '4m Gold', chance: 5, color: 'text-purple-500', icon: '👑' },
];
...
export default function Roleta() {
  ...
  const girar = useCallback(async () => {
    ...
    const interval = setInterval(() => {
        setSlots([
            PRIZES[Math.floor(Math.random() * PRIZES.length)].icon,
            PRIZES[Math.floor(Math.random() * PRIZES.length)].icon,
            PRIZES[Math.floor(Math.random() * PRIZES.length)].icon
        ]);
    }, 100);
  ...
  }, [spinning, girosDisponiveis, profile, refreshProfile]);

  return (
    <div className="min-h-screen bg-black text-stone-200 font-serif selection:bg-red-900/30">
      <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 space-y-8 animate-in fade-in duration-500">

        <header className="flex flex-col sm:flex-row items-center justify-between border-b border-white/10 pb-6 gap-4">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-red-900/20 rounded-lg text-red-500 border border-red-900/30 shadow-[0_0_15px_rgba(220,38,38,0.2)]">
