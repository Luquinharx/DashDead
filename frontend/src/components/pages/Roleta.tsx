...
import type { CasinoPrize } from '../../hooks/useCasinoConfig';
...
export default function Roleta() {
  ...
  const [result, setResult] = useState<CasinoPrize | null>(null);
  ...
  // Loot Reference for standard Casino
  ...
  const spinWheel = useCallback(async () => {
    ...
    const spins = 5 * 360; 
    const offsetAngle = 360 - prizeCenterAngle; 
  ...
  return (
    <div className="w-full text-stone-200 font-serif selection:bg-red-900/30 pt-4">
      <div className="w-full mx-auto space-y-10 animate-in fade-in duration-700">
        ...
        <header className="flex flex-col sm:flex-row items-center justify-between border-b border-white/10 pb-6 gap-4">
          <div className="flex items-center gap-5">
            <div className="p-4 bg-red-950/40 rounded-xl text-red-500 border border-red-500/30 shadow-[0_0_20px_rgba(220,38,38,0.3)] ring-1 ring-red-500/20">
              <Gift className="w-8 h-8 drop-shadow-[0_0_10px_rgba(220,38,38,0.8)]" />
            </div>
            <div>
              <h1 className="text-3xl md:text-5xl font-black text-white tracking-widest uppercase drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] font-sans">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-red-800">Blood</span> Wheel
              </h1>
              <p className="text-stone-400 font-medium tracking-widest uppercase mt-2 text-sm flex items-center gap-2">
                <Info className="w-4 h-4"/> ONE SPIN PER WEEK • QUALIFICATION: 5K+ LOOT
              </p>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-stone-950 border border-white/5 rounded-lg p-5 text-center shadow-2xl relative overflow-hidden group hover:border-red-500/30 transition-colors">
            <div className="absolute top-0 left-0 w-1 h-full bg-red-600/50"></div>
            <p className="text-xs text-stone-500 uppercase tracking-widest font-black">Last Week Loot</p>
            <p className="text-2xl font-black text-white mt-2 drop-shadow-md">
                {lootReference.toLocaleString('en-US')}
            </p>
          </div>
          
          <div className="bg-stone-950 border border-white/5 rounded-lg p-5 text-center shadow-2xl relative overflow-hidden group hover:border-red-500/30 transition-colors">
             <div className="absolute top-0 left-0 w-1 h-full bg-red-600/50"></div>
            <p className="text-xs text-stone-500 uppercase tracking-widest font-black">Status</p>
            <p className={cn(
                "text-xl font-black mt-2 uppercase tracking-widest", 
                isQualified ? "text-emerald-400 drop-shadow-[0_0_10px_rgba(52,211,153,0.4)]" : "text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.4)]"
            )}>
          ...
          <div className="bg-stone-950 border border-white/5 rounded-lg p-5 text-center shadow-2xl relative overflow-hidden group hover:border-red-500/30 transition-colors">
            <div className="absolute top-0 left-0 w-1 h-full bg-red-600/50"></div>
            <p className="text-xs text-stone-500 uppercase tracking-widest font-black">Spins Used</p>
            <p className="text-2xl font-black text-white mt-2 drop-shadow-md">
                {usedSpins} <span className="text-lg text-stone-600">/ {weeklyTotal}</span>
            </p>
          </div>

          <div className="bg-stone-950 border border-red-900/30 rounded-lg p-5 text-center shadow-[0_0_20px_rgba(220,38,38,0.1)] relative overflow-hidden">
             <div className="absolute top-0 left-0 w-1 h-full bg-red-500"></div>
            <p className="text-xs text-red-500 uppercase tracking-widest font-black">Extra Spins</p>
            <p className="text-3xl font-black text-red-500 mt-2 drop-shadow-[0_0_10px_rgba(220,38,38,0.5)]">
                {extraSpins}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-10 py-20 bg-gradient-to-b from-stone-950 to-black rounded-xl border border-white/5 relative overflow-hidden shadow-2xl">
            ...
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-red-900/20 via-black/90 to-black pointer-events-none"></div>

            {/* Turn Count Display */}
            <div className="z-10 bg-black/80 px-8 py-3 rounded-full border border-red-500/20 backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.8)]">
                <span className="text-red-400 font-bold uppercase tracking-widest text-sm flex items-center gap-2">
                    <Gift className="w-4 h-4" /> Available Spins: {availableSpins}
                </span>
            </div>
            ...
            <div className="relative w-[320px] h-[320px] sm:w-[480px] sm:h-[480px] flex items-center justify-center my-8 z-10">
                ...
                <div className="absolute -top-6 z-30 flex flex-col items-center filter drop-shadow-[0_8px_16px_rgba(0,0,0,1)]">
                    <div className="w-10 h-12 bg-gradient-to-b from-stone-100 to-stone-400 rounded-t-lg shadow-[inset_0_2px_4px_rgba(255,255,255,0.8)] border border-stone-400 border-b-none z-10 flex items-center justify-center">
                        <div className="w-2 h-2 rounded-full bg-stone-500 shadow-inner"></div>
                    </div>
                ...
                <div className="w-full h-full rounded-full relative bg-gradient-to-tr from-yellow-700 via-yellow-400 to-yellow-600 shadow-[0_30px_60px_rgba(0,0,0,0.9),inset_0_5px_15px_rgba(255,255,255,0.4)] border border-yellow-300/30 p-4 sm:p-6 flex items-center justify-center">
                        ...
                        {Array.from({length: 24}).map((_, i) => (
                            <div key={i} className="absolute origin-bottom" style={{ 
                               ...
                               <div className={cn(
                                   ...
                                   (spinning ? (i % 2 === 0 ? "bg-amber-100 animate-ping drop-shadow-[0_0_12px_rgba(255,255,255,1)]" : "bg-red-500 drop-shadow-[0_0_12px_rgba(239,68,68,1)]") : "bg-yellow-100/50")
                               )}></div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div 
                        className="w-full h-full rounded-full bg-stone-950 shadow-[0_0_50px_rgba(0,0,0,1)] relative overflow-hidden transition-transform ease-[cubic-bezier(0.1,0.1,0.05,1)] border-[12px] border-stone-800"
                        style={{ 
                    ...
                    >
                        ...
                        <div className="absolute inset-2 md:inset-4 rounded-full border border-stone-700 pointer-events-none z-0"></div>
                        ...
                        {config.prizes.map((p, i) => {
                            ...
                            return (
                                <div key={p.id} className="absolute origin-bottom flex flex-col items-center pt-8 sm:pt-10 z-10" style={{
                                    ...
                                    <div className={cn(
                                        "font-black text-[10px] sm:text-[13px] uppercase tracking-wider text-center leading-tight drop-shadow-lg text-white",
                                        // The config colors like 'text-amber-500' will apply from p.color if they exist. 
                                        p.color
                                    )} title={p.name}>
                        ...
                        {/* Center Hub Overlay */}
                        <div className="absolute inset-0 m-auto w-20 h-20 sm:w-28 sm:h-28 rounded-full bg-gradient-to-tr from-stone-900 to-stone-700 shadow-[0_0_30px_rgba(0,0,0,1)] border-[6px] border-stone-800 flex items-center justify-center z-20">
                            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-red-900 to-red-600 shadow-[inset_0_2px_10px_rgba(0,0,0,0.8)] flex items-center justify-center border-[3px] border-red-950/80">
                                <Gift className="w-6 h-6 sm:w-8 sm:h-8 text-red-200 drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {/* Spin Control Button */}
             <button
                ...
                className={cn(
                  ...
                  spinning
                    ...
                    : availableSpins > 0
                      ? "bg-gradient-to-b from-red-600 to-red-800 text-white hover:from-red-500 hover:to-red-700 hover:shadow-[0_0_60px_rgba(220,38,38,0.7)] border-red-950/80"
             ...
             {result && !spinning && (
                <div className="z-20 animate-in fade-in zoom-in duration-500 mt-8 text-center bg-stone-950/80 px-12 py-8 rounded-3xl border border-red-500/40 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] min-w-[300px]">
                    <div className="text-red-400 font-sans font-bold uppercase text-xs tracking-[0.4em] mb-4 flex items-center justify-center gap-2">
                      <Gift className="w-4 h-4" /> PRIZE WON <Gift className="w-4 h-4" />
                    </div>
                    <div className="flex flex-col items-center justify-center gap-6">
                      ...
                      <button onClick={() => setResult(null)} className="mt-4 px-8 py-2 border border-white/10 text-stone-400 uppercase tracking-widest text-xs rounded-full hover:bg-white/5 hover:text-white transition-colors">
                        Close
                      </button>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-16 w-full max-w-6xl px-6 opacity-90 z-10">
                {config.prizes.map((p) => (
                    <div key={p.id} className="flex flex-col items-center justify-center p-6 rounded-2xl bg-black/40 border border-white/5 backdrop-blur-xl group hover:border-red-600/40 hover:bg-red-900/10 transition-all duration-500">
                        ...
                        <div className="text-xs text-stone-400 mt-3 font-mono bg-black/50 px-4 py-1.5 rounded-full border border-white/5 shadow-inner">
                          {p.chance}% CHANCE
                        </div>
                    </div>
                ))}
        ...
        {/* History */}
        {history.length > 0 && (
          <div className="bg-stone-950 border border-white/10 rounded-2xl overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.6)]">
            <div className="px-8 py-6 border-b border-white/10 bg-gradient-to-r from-red-950/40 to-transparent flex items-center gap-3">
              <Gift className="w-5 h-5 text-red-500" />
              <h2 className="text-xl font-black text-white tracking-widest uppercase">Spin History</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm font-sans">
                <thead className="text-xs text-stone-400 uppercase bg-black/50 border-b border-white/5 tracking-wider">
                  <tr>
                ...
                <tbody className="divide-y divide-white/5">
                  {history.map((h, i) => (
                    <tr key={i} className="hover:bg-red-900/10 transition-colors group">   
                      <td className="px-8 py-6 text-stone-400 tracking-wide font-mono text-sm group-hover:text-stone-300">{h.date}</td>
